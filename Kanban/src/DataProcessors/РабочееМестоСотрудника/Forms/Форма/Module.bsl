...
&НаКлиенте
Процедура ПериодСрокВыполненияИспПриИзменении(Элемент) 
	УстановитьОтборПоСрокуВыполненияЗадачи(ПериодСрокВыполненияИсп);
КонецПроцедуры  
&НаКлиенте
Процедура УстановитьОтборПоСрокуВыполненияЗадачи (Период)
	   Если ЗначениеЗаполнено(Период) Тогда 
		ФильтрПериодСрокВыполнения = Период;
		СписокИсполнено.Отбор.Элементы.Очистить();
		ЭтотОбъект.ОбновитьОтображениеДанных();
		Компоновщик = СписокИсполнено.КомпоновщикНастроек;
